...
for brain in tqdm(brains):
    ...
    image_base_path = (
        rf"/mnt/g/AllenDataalignmentProj/resolutionPixelSizeMetadata/ISH/{brain}/"
    )
    ...
    for slice in alignment["slices"]:
        ...
        try:
            ...
            continue
        try:
            non_linear_data, non_linear_height, non_linear_width = read_nonlinear(
                non_linear_path
            )
        except:
        ...
        adjusted_image = apply_affine_to_image(
            image, affine_matrix, (non_linear_height, non_linear_width), mode="edge"
        )
