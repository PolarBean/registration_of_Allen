...
import DeepSlice.coord_post_processing
import DeepSlice.coord_post_processing.angle_methods
import DeepSlice.coord_post_processing.depth_estimation
import DeepSlice.coord_post_processing.spacing_and_indexing
import numpy as np
import os
import pandas as pd
...
for af in alignment_files:
    ...
    df = DeepSlice.coord_post_processing.angle_methods.propagate_angles(df, method="mean", species='mouse')
    maxNumber = df['nr'].max()
    if maxNumber < 250:
    ...
    DeepSlice.read_and_write.QuickNII_functions.write_QUINT_JSON(
        df, f"{path_to_images}/{animal_name}/{animal_name}", target=target, aligner="DeepSlice"
        )

    DeepSlice.read_and_write.QuickNII_functions.write_QUINT_JSON(
        df, f"human_corrected_registrations/{animal_name}", target=target, aligner="DeepSlice"
        )
